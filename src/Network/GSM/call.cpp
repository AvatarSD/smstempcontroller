 ...
 **********************************************************/
char CallGSM::CallStatus(void)
{
	char ret_val = CALL_NONE;

	if (GSM::CLS_FREE != gsm.getCOMStatus()) return (CALL_COMM_LINE_BUSY);
	gsm.setCOMStatus(GSM::CLS_ATCMD);
	gsm.SimpleWrite("AT+CPAS");

	// 5 sec. for initial comm tmout
	// 50 msec. for inter character timeout
	if (GSM::RX_TMOUT_ERR == gsm.WaitResp(5000, 50)) {
...
{
	if (GSM::CLS_FREE != gsm.getCOMStatus()) return;
	gsm.setCOMStatus(GSM::CLS_ATCMD);
	// ATDxxxxxx;<CR>
	gsm.SimpleWrite("ATD");
	gsm.print(number_string);
	gsm.SimpleWrite(";");
