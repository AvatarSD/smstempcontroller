	...
	for (i = 0; i < 1; i++)
	{
		...
		if(gsm.WaitResp(1000, 500, "OK") == GSM::RX_FINISHED_STR_RECV)
		{
			...
			if (GSM::RX_FINISHED_STR_RECV == gsm.WaitResp(1000, 500, ">")) {
				...
				if (GSM::RX_FINISHED_STR_RECV == gsm.WaitResp(7000, 5000, "+CMGS")) {
 ...
 **********************************************************/
char SMSGSM::IsSMSPresent(char required_status)
{
	char ret_val = -1;
	char *p_char;
	char status;

	if (GSM::CLS_FREE != gsm.getCOMStatus()) return (ret_val);
 ...
 **********************************************************/
char SMSGSM::GetSMS(char position, char *phone_number, char *SMS_text, unsigned char max_SMS_len)
{
	char ret_val = -1;
	char *p_char;
	char *p_char1;
 ...
 **********************************************************/
char SMSGSM::GetAuthorizedSMS(char position, char *phone_number, char *SMS_text, char max_SMS_len,
...
char SMSGSM::InitSMSMemory(void)
{
	...
	if (GSM::AT_RESP_OK == gsm.SendATCmdWaitResp("AT+CPMS=\"SM\",\"SM\",\"SM\"", 1000, 1000, "+CPMS:", 10)) {
		ret_val = 1;
	}
	else ret_val = 0;
