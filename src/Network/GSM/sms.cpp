...
char SMSGSM::SendSMS(const char *number_str, const char *message_str)
{
	...
	for (i = 0; i < 1; i++)
	{
		gsm.SimpleWrite("AT+CMGF=1");
		gsm.print("\r");
		if (gsm.WaitResp(1000, 500, "OK") == GSM::RX_FINISHED_STR_RECV)
		{
			...
			if (GSM::RX_FINISHED_STR_RECV == gsm.WaitResp(1000, 500, ">"))
			{
#ifdef DEBUG_ON
				Serial.println("DEBUG:>");
#endif
				// send SMS text
				gsm.print(message_str);
				gsm.SimpleWrite(end);
				gsm.print("\r");
				//gsm.flush(); // erase rx circular buffer
				if (GSM::RX_FINISHED_STR_RECV
						== gsm.WaitResp(7000, 5000, "+CMGS"))
				{
 ...
 **********************************************************/
char SMSGSM::IsSMSPresent(sms_type_enum required_status)
{
	char ret_val = -1;
	char *p_char;
	char status;

	if (GSM::CLS_FREE != gsm.getCOMStatus())
		return (ret_val);
	...
	case SMS_UNREAD:
		...
		break;
	case SMS_READ:
		gsm.print("AT+CMGL=\"REC READ\"");
		gsm.print("\r");
		break;
	case SMS_ALL:
		gsm.print("AT+CMGL=\"ALL\"");
		gsm.print("\r");
		break;
	}
 ...
 **********************************************************/
SMSGSM::getsms_ret_val_enum SMSGSM::GetSMS(char position, char *phone_number,
		char *SMS_text, unsigned char max_SMS_len)
{
	getsms_ret_val_enum ret_val = GETSMS_NOT_AUTH_SMS;
	char *p_char;
	char *p_char1;
	char len;
 ...
 **********************************************************/
SMSGSM::getsms_ret_val_enum SMSGSM::GetAuthorizedSMS(char position,
		char *phone_number, char *SMS_text, char max_SMS_len,
		char first_authorized_pos, char last_authorized_pos)
{
	getsms_ret_val_enum ret_val = GETSMS_ERROR;
	char i;
...
/**********************************************************
 ...
 1 - SMS was deleted
 **********************************************************/
char SMSGSM::DeleteSMS(char position)
{
	char ret_val = -1;
...
char SMSGSM::InitSMSMemory(void)
{
	char ret_val = -1;
...
//	gsm.SendATCmdWaitResp("AT+CMGF=1", 1000, 50, "OK", 2);
	...
	if (GSM::AT_RESP_OK
			== gsm.SendATCmdWaitResp("AT+CPMS=\"SM\",\"SM\",\"SM\"", 1000, 1000,
					"+CPMS:", 10))
	{
		ret_val = 1;
	}
